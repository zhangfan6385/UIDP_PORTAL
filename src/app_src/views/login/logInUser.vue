<template>
  <div class="social-signup-container">
     ...
     <el-table :data="list" :header-cell-class-name="tableRowClassName"   @current-change="handleCurrentChange" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      style="width: 100%">
         <el-table-column width="170px" align="center" label="员工账户">
        <template slot-scope="scope">
          <span>{{scope.row.USER_DOMAIN}}</span>
        </template>
      </el-table-column>
       <el-table-column width="170px" align="center" label="员工姓名">
        <template slot-scope="scope">
          <span>{{scope.row.USER_NAME}}</span>
        </template>
      </el-table-column>
      
       <el-table-column width="170px" align="center" label="员工编号">
        <template slot-scope="scope">
